...
def performVAD(src):
    ...
    for i, s in enumerate(st):
        ...
        chunklist.append(
            {'start': s['start'] - int(120 * SAMPLING_RATE / 1000), 'end': s['end'] + int(50 * SAMPLING_RATE / 1000),
             'idx': i, 'text': "", 'fname': fname, 'speaker': ""})
...
def doTranscribe(src, ms, dv):
    ...
    subs = SubRipFile()
    options = {"task": "transcribe", "language": "English", "fp16": fp16, 'no_speech_threshold': 0.1,
    ...
    for s in st:
        for sp in splist:
            ...
            if s['start'] / SAMPLING_RATE >= sp['start'] - 0.2 and s['end'] / SAMPLING_RATE <= sp['end'] + 0.3:
                print(
                    Fore.GREEN + f"{s['idx']=}:  {s['start']/SAMPLING_RATE=}  {sp['start']-0.2=}   | {s['end']/SAMPLING_RATE=}  {sp['end']+0.2=}")
...
def TranscriptionPipe(src, modelsize, device):
    ...
    localdiarise("work/" + os.path.splitext(os.path.basename(src))[0] + "_temp.wav",
                 "work/" + os.path.splitext(os.path.basename(src))[0] + "_diarise.json")
...
def Interactive():
    layout = [
        ...
        [sg.Radio('tiny.en', 'Model', key="tiny.en"), sg.Radio('base.en', 'Model', key="base.en"),
         sg.Radio('small.en', 'Model', key="small.en"), sg.Radio('medium.en', 'Model', key="medium.en", default=True),
         sg.Radio('large-v1', 'Model', key="large-v1"), sg.Radio('large-v2', 'Model', key="large-v2")],
